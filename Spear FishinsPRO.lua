...
-- ฟังก์ชันยิง Remote
local function fireHit(target, tool)
	...
	local args = {
		...
		{
			fishInstance = target,
			HitPos = target:GetPivot().Position, -- ตำแหน่งอัตโนมัติ
			toolInstance = tool
		}
	}
...
local Toggle = Tab:Toggle({
	...
	Callback = function(state)
		...
		if state then
			task.spawn(function()
				while running do
					-- 🔍 ตรวจ WorldBoss.Point1
					local point1 = workspace:FindFirstChild("WorldBoss")
						and workspace.WorldBoss:FindFirstChild("Point1")

					if point1 then
						for _, target in ipairs(point1:GetChildren()) do
